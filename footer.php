</div><!-- /.row -->

</div><!-- /.container -->
<footer class="blog-footer">
    <section class="footer">
        <div class="container">
            <div class="row">
                <div class="d-none d-md-block col-md-3">
                    <div class="footer_icon">
                        <img src="<?php bloginfo('template_directory') ?>/img/jt_logo_monochrome.svg"
                            alt="footer logo" />
                        <br /><br />
                        <p>enjoy the move</p>
                    </div>
                </div>
                <!-- services -->
                <div class="d-none d-md-block col-md-3 services-footer-links">
                    <h5>Services</h5>
                    <br />
                    <ul>
                        <li>
                            <a href="https://joeltransport.mmogomedia.com/home-moves">Domestic Moves</a>
                        </li>
                        <li>
                            <a href="https://joeltransport.mmogomedia.com/home-moves#international_move">International
                                Moves</a>
                        </li>
                        <li>
                            <a href="https://joeltransport.mmogomedia.com/home-moves#domestic_moves">Home Moves</a>
                        </li>
                        <li>
                            <a href="https://joeltransport.mmogomedia.com/office-removals">Office Moves</a>
                        </li>
                        <li>
                            <a href="https://joeltransport.mmogomedia.com/home-moves#office_storage">Storage</a>
                        </li>
                        <li>
                            <a href="https://joeltransport.mmogomedia.com/home-moves#insurance">Insurance</a>
                        </li>
                        <br>
                        <li>
                            <a href="https://joeltransport.mmogomedia.com/home-moves#specialized">Specialized
                                services</a>
                        </li>
                        <li>
                            <a href="https://joeltransport.mmogomedia.com/home-moves#specialized">Pets</a>
                        </li>
                        <li>
                            <a href="https://joeltransport.mmogomedia.com/home-moves#specialized">Pianos</a>
                        </li>
                        <li>
                            <a href="https://joeltransport.mmogomedia.com/home-moves#specialized">Fine art</a>
                        </li>
                        <li>
                            <a href="https://joeltransport.mmogomedia.com/home-moves#specialized">Vehicle transport</a>
                        </li>
                        <li>
                            <a href="https://joeltransport.mmogomedia.com/home-moves#specialized">Dangerous goods</a>
                        </li>
                    </ul>
                </div>
                <!-- resources -->
                <div class="d-none d-md-block col-md-3 resources-footer-links">
                    <h5>Resources</h5>
                    <br />
                    <ul>
                        <li>
                            <a href="/resources#faqs">FAQs</a>
                        </li>
                        <li>
                            <a href="/resources#moving_tips">Moving tips</a>
                        </li>
                        <li>
                            <a href="/resources#home_checklist">Home moves checklist</a>
                        </li>
                        <li>
                            <a href="/resources#office_checklist">Office moves checklist</a>
                        </li>
                    </ul>
                </div>
                <!-- contact -->
                <div class="col-md-3">
                    <h5>Contact</h5>
                    <br />
                    <ul>
                        <li>Monday - Friday</li>
                        <li>8am - 5pm</li>
                    </ul>

                    <!-- address -->
                    <p>
                        10 Von Tonder Street
                        <br />
                        Sunderland Ridge
                        <br />
                        Centurion, 0157
                        <br />
                        South Africa
                    </p>
                    <!-- email -->
                    <ul>
                        <li class="text-bold">email</li>
                        <li><a href="mailto:joeltransport.co.za">sales@joeltransport.co.za</a></li>
                    </ul>
                    <!-- phone -->
                    <ul>
                        <li class="text-bold">phone</li>
                        <li><a href="tel:27677778821">+27 12 666 9055</a></li>
                    </ul>
                    <ul>
                        <li class="text-bold">fax</li>
                        <li><a href="tel:27677778821">+27 12 666 9186</a></li>
                    </ul>
                </div>
            </div>
        </div>
        <nav class="nav justify-content-center footer_nav pb-3 d-none d-md-flex col-md-12">
            <li class="nav-item">
                <a class="nav_link" href="/about">About</a>
            </li>
            <li class="nav-item">
                <a class="nav_link" href="/about#community">Community</a>
            </li>
            <li class="nav-item">
                <a class="nav_link" href="/about#careers">Careers</a>
            </li>
            <li class="nav-item">
                <a class="nav_link" href="/contact#contact">Contact</a>
            </li>
            <li class="nav-item">
                <a class="nav_link" href="/policies#removal_policy">Removal Policy</a>
            </li>
            <li class="nav-item">
                <a class="nav_link" href="/policies#privacy_policy">Privacy Policy</a>
            </li>
            <li class="nav-item">
                <a class="nav_link" href="/resources#terms">Terms & conditions</a>
            </li>
        </nav>
        <div class="col-12 d-md-none">
            <div class="footer_icon">
                <img src="<?php bloginfo('template_directory') ?>/img/jt_logo_monochrome.svg" alt="footer logo" />
                <br /><br />
                <p>enjoy the move</p>
            </div>
        </div>
        <div class="footer_bottom d-flex">
            <div class="copyright text-center">
                <p>Copyright&copy; <?php echo Date('Y'); ?> - <?php bloginfo('name'); ?><span
                        class="d-none d-md-block">. All Rights Reserved</span></p>
            </div>

        </div>
    </section>

</footer>
<script src="https://code.jquery.com/jquery-2.2.4.min.js"></script>
<?php wp_footer(); ?>


</body>

</html>